...
const MealCards = () => {
    ...
    useEffect(() => {
        async function fetchCity() {
            try {
                const res = await fetch(`/api/cities?slug=${currentCity}`);
                if (res.ok) {
    ...
    const handleFilterChange = (filterSlug) => setActiveFilter(filterSlug);
    const handleReserveTable = (meal) => router.push(`/reservation/${meal.slug}?city=${currentCity}`);
    ...
    const renderMealCards = (mealsToRender) => (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 ">
            {mealsToRender.map((meal) => (
                <div key={meal._id || meal.id} className="bg-white rounded-tl-2xl rounded-br-2xl shadow-lg overflow-hidden hover:shadow-xl transition-shadow duration-300">
                    ...
                    <div className="p-6">
                        ...
                        <div className="flex gap-3">
                            <button className="flex-1 border py-2 px-4 rounded-tl-2xl rounded-br-2xl hover:bg-[rgba(138,23,57,0.1)] transition-colors duration-200 text-sm font-playfair font-medium" style={{ borderColor: "#8A1739", color: "#8A1739" }}>
                                View Menu
                            </button>
                            <button onClick={() => handleReserveTable(meal)} className="flex-1 py-2 px-4 rounded-tl-2xl rounded-br-2xl hover:bg-[#8A1739]/90 transition-colors duration-200 text-sm font-playfair font-medium text-white" style={{ backgroundColor: "#8A1739" }}>
                                Reserve Table Now
                            </button>
                        </div>
                    </div>
            ...
            <div className="container mx-auto px-4">
                ...
                {city && (
                    <div className="mb-12 text-center">
                        ...
                        <div className="mt-2 text-[#D4AF37] font-medium text-lg">
                            {city.description}
                        </div>
                    </div>
                )}
                ...
                <div className="flex flex-wrap justify-end gap-2 md:gap-3 mb-12">
                    {filters.map((filter) => (
                        <button
                            ...
                            className={`px-6 py-3 rounded-tl-2xl rounded-br-2xl font-medium transition-all duration-300 text-sm md:text-base ${activeFilter === filter.slug
